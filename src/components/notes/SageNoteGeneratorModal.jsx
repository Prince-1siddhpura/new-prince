...
export const SageNoteGeneratorModal = ({
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) {
      setErrorMessage('Please enter a topic or instruction for Sage AI.');
      return;
    }
    setErrorMessage('');
    ...
    try {
      ...
      setErrorMessage(err.message || 'Unable to generate notes with Sage AI.');
      console.error(err);
    } finally {
      setLoading(false);
    }
      ...
      <div style={{
        ...
        <div style={{ padding: '24px 28px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div>
            ...
            {errorMessage && (
              <div style={{ color: '#ef4444', fontSize: '0.8rem', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
