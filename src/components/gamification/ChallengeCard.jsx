...
export const ChallengeCard = ({ challenge, onClaimed }) => {
  const { earnXp } = useLearning();

  const handleClaim = async () => {
    earnXp(challenge.xpReward);
    try {
      if (challenge.id && !challenge.id.startsWith('ch_')) {
        await gamificationApi.completeMission(challenge.id);
      } else {
        await gamificationApi.addXp(challenge.xpReward, `Daily Quest: ${challenge.title}`);
      }
    } catch (e) {
      gamificationApi.addXp(challenge.xpReward, `Daily Quest: ${challenge.title}`).catch(() => {});
    }
    if (onClaimed) onClaimed(challenge.id);
  };
    ...
    <Card hoverEffect style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
      ...
      <div>
        ...
        >
          ...
          {challenge.completed ? `Claimed (+${challenge.xpReward} XP)` : `Claim +${challenge.xpReward} XP Reward`}
        </Button>
      </div>
    </Card>
  );
