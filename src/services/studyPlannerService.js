...
import { analyticsApi, learnerApi } from '../lib/apiClient';
...
class StudyPlannerService {
  ...
  async syncWithBackend() {
    try {
      const [sessionsRes, goalsRes] = await Promise.allSettled([
        analyticsApi.getStudySessions(),
        learnerApi.getGoals(),
      ]);

      if (sessionsRes.status === 'fulfilled' && sessionsRes.value?.success && Array.isArray(sessionsRes.value?.data)) {
        if (sessionsRes.value.data.length > 0) {
          this.sessions = sessionsRes.value.data.map((dbSession) => ({
            id: dbSession.id,
            planId: this.currentPlan?.id || 'plan_db',
            userId: dbSession.userId,
            day: new Date(dbSession.plannedDate).toLocaleDateString('en-US', { weekday: 'long' }),
            date: new Date(dbSession.plannedDate).toISOString().split('T')[0],
            startTime: '07:00 PM',
            endTime: '08:00 PM',
            slot: 'Evening',
            subjectId: dbSession.subjectId,
            subjectName: dbSession.subject?.name || 'Subject',
            subjectColor: '#6366f1',
            chapter: 'Chapter 1',
            topic: `${dbSession.subject?.name || 'Subject'} Review`,
            durationMinutes: dbSession.durationMinutes,
            hours: (dbSession.durationMinutes / 60).toFixed(1),
            studyType: 'Practice',
            priority: 'High',
            status: dbSession.completed ? 'Completed' : 'Upcoming',
            progress: dbSession.completed ? 100 : 0,
            source: 'manual',
            completedAt: dbSession.completed ? dbSession.updatedAt : null,
            materials: [],
          }));
          this.saveState();
        }
      ...
      if (goalsRes.status === 'fulfilled' && goalsRes.value?.success && Array.isArray(goalsRes.value?.data)) {
        if (goalsRes.value.data.length > 0) {
          this.goals = goalsRes.value.data;
          this.saveGoals();
        }
      }
    } catch (e) {
      console.warn('Could not sync study planner with backend', e);
    }
  }

  // Add Study Session (persisted to PostgreSQL)
  addSession(sessionData) {
    ...
    // Asynchronously persist to backend PostgreSQL if subject is selected
    if (sessionData.subjectId && !sessionData.subjectId.startsWith('sub_')) {
      analyticsApi.createStudySession({
        subjectId: sessionData.subjectId,
        durationMinutes: newSession.durationMinutes,
        plannedDate: newSession.date,
      }).then((res) => {
        if (res?.success && res.data?.id) {
          newSession.id = res.data.id;
          this.saveState();
        }
      }).catch((err) => {
        console.warn('Could not persist study session to PostgreSQL:', err);
      });
    }
  ...
  deleteSession(sessionId) {
    // If it's a persistent backend session, delete from PostgreSQL
    if (sessionId && !sessionId.startsWith('sess_manual_') && !sessionId.startsWith('sess_plan_')) {
      analyticsApi.deleteStudySession(sessionId).catch((err) => {
        console.warn('Could not delete study session from backend:', err);
      });
    }
  ...
  // Complete session & record feedback (persisted to PostgreSQL with real XP)
  completeStudySession(sessionId, feedback = {}) {
    ...
    if (sess) {
      ...
      sess.difficultyRating = feedback.difficulty || 'Okay';
      sess.confidenceLevel = feedback.confidence || 4;
      this.saveState();

      // Persist completion and real XP transaction to PostgreSQL
      if (sessionId && !sessionId.startsWith('sess_manual_') && !sessionId.startsWith('sess_plan_')) {
        analyticsApi.completeStudySession(sessionId).then((res) => {
          if (res?.success && res.data?.xpEarned) {
            earnedXP = res.data.xpEarned;
          }
        }).catch((err) => {
          console.warn('Could not complete study session on server:', err);
        });
      }
    }
