...
export const ProfilePage = () => {
  ...
  const handleImageFileChange = (e) => {
    ...
    if (file) {
      if (!file.type.startsWith('image/')) {
        setCopyNotice('⚠ Please select a valid image file (JPG, PNG, WEBP, GIF).');
        setTimeout(() => setCopyNotice(''), 3500);
        return;
      }
      if (file.size > 5 * 1024 * 1024) {
        setCopyNotice('⚠ Image size exceeds 5MB limit. Please select a smaller photo.');
        setTimeout(() => setCopyNotice(''), 3500);
        return;
      }
  ...
  const handleSaveProfile = async () => {
    if (usernameStatus && !usernameStatus.available && username !== (learner?.username || user?.studentUsername || 'alexmercer')) {
      setCopyNotice('⚠ Please select an available username before saving.');
      setTimeout(() => setCopyNotice(''), 3500);
      return;
    }
