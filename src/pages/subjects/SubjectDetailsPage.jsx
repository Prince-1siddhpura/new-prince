...
import { noteApi } from '../../lib/apiClient';
import {
...
export const SubjectDetailsPage = () => {
  ...
  const [subject, setSubject] = useState(() => {
    const s = subjectService.getSubjectById(subjectId);
    return s || {
      id: subjectId,
      ...
      currentChapter: 'Core Concepts',
      weakTopic: 'None',
      strongTopic: 'Foundations'
    };
  });
  ...
  const topics = (Array.isArray(subject?.topics) && subject.topics.length > 0) ? subject.topics : [];
  ...
  const handleSaveNote = (e) => {
    ...
    if (editingNoteId) {
      ...
      // Also persist to PostgreSQL backend Smart Notes
      noteApi.createNote({
        title: `${subject.name}: ${noteTitle}`,
        content: noteContent,
        category: subject.name,
        tags: [subject.name, 'Subject Notes']
      }).catch(err => {
        console.warn('Could not sync subject note to backend:', err.message);
      });
    }
