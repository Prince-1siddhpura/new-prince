...
export const ParentRegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/verify-otp?email=${encodeURIComponent(parentEmail.trim())}`, {
        state: {
          email: parentEmail.trim(),
          message: '✓ Guardian account created! Enter the 6-digit confirmation code dispatched via SMTP to activate access.'
        }
      });
    } catch (err) {
      console.error(err);
