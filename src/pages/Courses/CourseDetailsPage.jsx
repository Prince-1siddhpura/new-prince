      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', inset: 0, padding: '32px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', maxWidth: '750px' }}>
            Instructor: <strong>{course.instructor?.name || (typeof course.instructor === 'string' ? course.instructor : 'EduNova Senior Instructor')}</strong> • {course.duration || '4.5 Hours'} Total Duration
          </p>
        </div>
      </div>
      ...
      <div className="dashboard-grid">
          ...
          <Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {(course.modules && course.modules.length > 0
                ? course.modules
                : [
                    { id: 'm1', title: `${course.title}: Core Architectural Foundations`, duration: '45 mins' },
                    { id: 'm2', title: 'Hands-on Implementation & Practical Patterns', duration: '1.5 Hours' },
                    { id: 'm3', title: 'Production Hardening & Knowledge Evaluation', duration: '1 Hour' }
                  ]
              ).map((m, idx) => (
                <div
                  key={m.id || idx}
                ...
                >
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{m.duration ? (typeof m.duration === 'number' ? `${m.duration} mins` : m.duration) : '45 mins'}</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="col-span-4" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <Card>
            ...
            <ProgressBar progress={course.progress || 0} height={8} />
