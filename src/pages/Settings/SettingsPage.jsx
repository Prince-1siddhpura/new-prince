...
import {
  ...
  Trash2, VolumeX, EyeOff, Palette, Monitor, Laptop, CheckSquare, Copy
} from 'lucide-react';
...
export const SettingsPage = () => {
  ...
  const [studentLinkCode, setStudentLinkCode] = useState(null);
  const [studentLinkExpires, setStudentLinkExpires] = useState(null);
  const [isGeneratingCode, setIsGeneratingCode] = useState(false);
  const [isCopied, setIsCopied] = useState(false);

  const handleGenerateStudentLinkCode = async () => {
    setIsGeneratingCode(true);
    try {
      const res = await authApi.generateStudentLinkCode();
      if (res.data?.linkCode) {
        setStudentLinkCode(res.data.linkCode);
        setStudentLinkExpires(res.data.expiresAt);
        showToast('✓ Generated 6-digit Parent Link PIN!');
      }
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to generate link code');
    } finally {
  ...
  const handleCopyLinkCode = () => {
    ...
    setIsCopied(true);
    showToast('✓ Link PIN copied to clipboard');
  ...
  const handleSaveUsername = async (e) => {
    ...
    if (usernameCheckStatus && !usernameCheckStatus.available && newUsernameInput !== username) {
      showToast('⚠ Please enter an available username.');
      return;
    }
    ...
    <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '50px' }}>
      ...
      <div id="category-tabs-section" style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px', borderBottom: '1px solid var(--border-color)' }}>
        {[
          ...
          ...((activeType === 'school' || user?.role === 'STUDENT' || !user?.role) ? [{ id: 'parent', label: 'Parent Companion', icon: HeartHandshake }] : []),
      ...
      {/* 7. PARENT COMPANION PANEL (For School & Student users) */}
      {activeCategory === 'parent' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <Card style={{ borderLeft: '4px solid #f59e0b' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <HeartHandshake size={18} style={{ color: '#f59e0b' }} /> Parent / Guardian Secure Linking
            </h3>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '16px', lineHeight: 1.6 }}>
              EduNova protects student privacy by requiring explicit two-factor student authorization. Parents must provide both your student username and a single-use 6-digit PIN generated from this panel to link their account.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', marginBottom: '20px' }}>
              <div style={{ background: 'var(--bg-tertiary)', padding: '16px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Your Student Username</span>
                <p style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--accent-cyan)', marginTop: '4px', fontFamily: 'monospace' }}>
                  {username || user?.username || 'alexmercer'}
                </p>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Share this with your parent</span>
              </div>

              <div style={{ background: 'var(--bg-tertiary)', padding: '16px', borderRadius: '10px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Single-Use Verification PIN</span>
                  {studentLinkCode ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '6px' }}>
                      <span style={{ fontSize: '1.4rem', fontWeight: 900, color: '#34d399', letterSpacing: '3px', fontFamily: 'monospace' }}>
                        {studentLinkCode}
                      </span>
                      <button
                      ...
                      >
                        {isCopied ? <Check size={14} /> : <Copy size={14} />} {isCopied ? 'Copied' : 'Copy PIN'}
                      </button>
                    </div>
                  ) : (
                    <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginTop: '6px' }}>
                      No active PIN generated.
                    </p>
                  )}
                </div>
                ...
                <div style={{ marginTop: '12px' }}>
                  <Button
                    ...
                    onClick={handleGenerateStudentLinkCode}
                  >
                    <Key size={14} /> {studentLinkCode ? 'Regenerate New PIN' : 'Generate Secure PIN'}
                  </Button>
                </div>
              </div>
            </div>
            ...
            {studentLinkCode && (
              <div style={{ padding: '12px 16px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.25)', borderRadius: '8px', fontSize: '0.82rem', color: '#fbbf24' }}>
                ⏱ <strong>PIN Expiration:</strong> This PIN is valid for 24 hours. Once your parent enters it, it is permanently invalidated to prevent reuse.
              </div>
            )}
          </Card>

          <Card>
            <h4 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '12px' }}>Current Linked Parent Status</h4>
            {parentCompanion.connected && parentCompanion.parentEmail ? (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', background: 'var(--bg-tertiary)', borderRadius: '8px' }}>
                <div>
              ...
              <div style={{ padding: '18px', textAlign: 'center', background: 'var(--bg-tertiary)', borderRadius: '8px', color: 'var(--text-secondary)', fontSize: '0.88rem' }}>
                No parent or guardian account is currently linked to your student profile.
              </div>
            )}
          </Card>
        </div>
