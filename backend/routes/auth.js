...
const emailVerificationRequestSchema = {
  body: z.object({
    email: z.string().email('Valid email is required').optional(),
  }),
};

const emailVerificationConfirmSchema = {
  body: z.object({
    email: z.string().email('Valid email is required').optional(),
...
const linkParentSchema = {
  body: z.object({
    ...
    linkCode: z.string().min(4, 'Verification link code from student is required'),
  }),
};
...
router.post('/verify-email/request', validate(emailVerificationRequestSchema), requestEmailVerification);
router.post('/verify-email/confirm', validate(emailVerificationConfirmSchema), confirmEmailVerification);

// Google
router.post('/google', validate(googleLoginSchema), googleLogin);

// Parent Linking Verification Workflow
router.post('/student-link-code', requireAuth, requireRole('STUDENT'), generateStudentLinkCode);
