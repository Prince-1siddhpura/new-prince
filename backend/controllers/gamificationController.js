...
const addXp = async (req, res) => {
  try {
    ...
    if (safeAmount <= 0) {
      return res.status(400).json({ success: false, message: 'XP amount must be a positive number' });
    }

    // Security: Only administrators can grant uncapped arbitrary XP.
    // Client-initiated learning activity awards are capped to maximum 50 XP per call.
    if (req.user?.role !== 'ADMIN') {
      safeAmount = Math.min(safeAmount, 50);
    }
